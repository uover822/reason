...
describe('reason', function () {
  ...
  it('query', {timeout: 8888}, function (done) {
    ...
    // Load the info plugin
      ...
      .act('role:reason,cmd:query,name:seneca', function (ignore, out) {
      ...
      .act('role:reason,cmd:get,name:seneca', function (ignore, out) {
        expect(out.id).to.equal('seneca')
        expect(out.giturl).to.contain('senecajs/seneca.git')
      ...
      .act('role:reason,cmd:get,name:seneca,update:true', function (ignore, out) {
        expect(out.id).to.equal('seneca')
        expect(out.giturl).to.contain('senecajs/seneca.git')
